...
@Component
public class VendedorDialog extends JDialog {
	...
	private VendedorInterfaz interfaz;
	
	private List<Usuario> vendedores;

	@Autowired
	public VendedorDialog(UsuarioService service, UsuarioTableModel tableModel) {
	...
	private void loadClients(String name) {
		if (name.isEmpty()) {
			...
			vendedores = service.findByUsuario(name);
		}
		
        tableModel.clear();
        tableModel.addEntities(vendedores);
    }
	
	public VendedorInterfaz getInterfaz() {
		return interfaz;
	}
	
	public void setInterfaz(VendedorInterfaz interfaz) {
		this.interfaz = interfaz;
	}
	
	private void aceptar() {
		for (Integer c : table.getSelectedRows()) {
			interfaz.getEntity(vendedores.get(c));         
	    }
		dispose();
	}
