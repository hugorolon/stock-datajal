...
@Component
public class NcmValidator extends ValidationSupport implements Validator<Ncm> {

    private static final String REQUIRED_DATA_NOT_FILLED_OR_BAD_DATA = "Campos Obligatorios";

    @Override
    public Optional<ValidationError> validate(Ncm ncm) {
        if (isNullOrEmptyString(ncm.getNombre())) {
